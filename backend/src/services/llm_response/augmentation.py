...
def stitched_content_formatter(
    ...
    if document.type == DocumentType.PDF:  # PDF
...
- Page: {head_chunk.document_page_num + document.page_starts_at} -> {page_end}
Contents:
{stitched_content}
"""

    elif document.type == DocumentType.IMAGE:  # Image
        return f"""Context {index}:
Source:
- Document name: {document.name}
    ...
    for chunks in grouped_docs.values():
        ...
        for j in range(1, len(chunks)):
            ...
            # If no gap, but we just crossed a page
                ...
                is_pdf
                and chunks[j].document_page_num != chunks[j - 1].document_page_num
            ):
                new_page = (
...
def prompt_augmentation(
    ...
    stitched_contents = stitcher(document_chunks)
    ...
    # Augmented prompt
    final_prompt = f"""=== PURPOSE AND SCOPE ===
